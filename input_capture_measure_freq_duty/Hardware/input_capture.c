...
void ic_init(void)
{
	...
	tim_ic_init.TIM_ICFilter = 0xF;  // 滤波器
	tim_ic_init.TIM_ICPolarity = TIM_ICPolarity_Falling;  // 极性选择: 下降沿
	tim_ic_init.TIM_ICPrescaler = TIM_ICPSC_DIV1;  // 输入采样分频
	tim_ic_init.TIM_ICSelection = TIM_ICSelection_IndirectTI;  // 指定触发信号的输入引脚
	TIM_ICInit(TIM3, &tim_ic_init);	
	
	
	// 6. 选择从模式触发源
	TIM_SelectInputTrigger(TIM3, TIM_TS_TI1FP1);  // DirectTI
